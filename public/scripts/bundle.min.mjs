const t=t=>{const e=document.querySelector("#data-backdrop"),i=e.getContext("2d"),s=window.innerWidth,n=window.innerHeight;e.width=2*s*window.devicePixelRatio,e.height=2*n*window.devicePixelRatio,e.style.width=s+"px",e.style.height=n+"px";const r=t.map((t=>t.icuOccupation.value)),o=t.map((t=>t.hospitalized7Days.value)),d=(t,s,n)=>{const r=t.length-1,o=e.width/r,d=e.height/n;i.beginPath(),i.strokeStyle=s,i.lineWidth=10,i.lineCap="round",i.lineJoin="round",i.moveTo(0,e.height-t[0]*d),t.forEach(((t,s)=>{i.lineTo(s*o,e.height-t*d)})),i.stroke(),i.strokeStyle="transparent",i.lineTo(e.width,e.height),i.lineTo(0,e.height),i.fillStyle=s,i.fill()};window.requestAnimationFrame((()=>{i.clearRect(0,0,e.width,e.height),d(r,"rgba(255, 90, 95, 0.05)",600),d(o,"rgba(57, 160, 237, 0.05)",1200)}))},e=({block:t=4,group:e=4}={})=>{const i="abcdefghijklmnopqrstuvxyz0123456789",s=i.length,n=[];for(let r=0;r<e;r++){let e=[];for(let n=0;n<t;n++)e.push(i[1*Math.floor(Math.random()*s)]);n.push(e.join(""))}return n.join("-")},i=new DOMParser;class s{constructor(){this.id=e(),this._selector=`.widget[widget-id='${this.id}']`}renderValue(t,e,i){const s=this.html,[n,r]=t.toString().split(".");let o="var(--widget-color-light)";e&&(o=t>e?"var(--widget-color-red)":"var(--widget-color-blue)"),600===e&&t>450&&(o="var(--widget-color-yellow)");return s`<h3
			class="widget-content__trend-value"
			style="color: ${o};  margin-left: auto; font-size: ${n.length>3?"3.5em":"5em"};"
		>
			${n}<span class="widget-content__trend-decimal"
				>${r?"."+r:""}${this.renderUnit(i)}</span
			>
		</h3>`}getScheme(t){return t?["var(--widget-color-blue)","var(--widget-color-red)"]:["var(--widget-color-red)","var(--widget-color-blue)"]}renderTrend(t,e){const i=this.html,{arrowRight:s,arrowRightUp:n,arrowRightDown:r}=Pangolicons.icons,o={"stroke-width":2,width:"8em",height:"8em"},d="vaccination"===this.widgetName?this.getScheme(!0):this.getScheme(!1);return i`<span class="widget-content__trend-indicator">
			${t>e?n.toString({...o,stroke:d[0]}):t===e?s.toString({...o,stroke:"var(--widget-color-light)"}):r.toString({...o,stroke:d[1]})}
		</span>`}renderUnit(t){return null===t||"Fälle"===t||void 0===t?"":t}renderDetails({value:t,lastValue:e,threshold:i,unit:s}){return`${this.renderDifference(t,e,s)}\n\t\t\t${i?this.renderPecentage(t,i):""}\n\t\t\t${i?this.renderTreshold(t,i):""}\n\t\t\t`}renderTreshold(t,e){return`<span\n\t\t\tclass="widget-details__threshold"\n\t\t\tstyle="color: ${t>e?"var(--widget-color-red)":"var(--widget-color-light)"}"\n\t\t\t>${t} / ${e}</span\n\t\t>`}renderPecentage(t,e){return`<span class="widget-details__percentage">\n\t\t\t(${(100*t/e).toFixed(2)}%)\n\t\t</span>`}renderDifference(t,e,i){const s=t-e,n="vaccination"===this.widgetName?this.getScheme(!0):this.getScheme(!1);return`<span class="widget-details__difference" style="color: ${t>e?n[0]:t<e?n[1]:"var(--widget-color-light)"}">${s<0?"":"+"} ${Number.isInteger(s)?s:s.toFixed(2)} ${this.renderUnit(i)}</span>`}html(t,...s){const n={},r=t.map(((i,r)=>{if(r<t.length-1){if(s[r]instanceof DocumentFragment){const t=e();return n[t]={frag:s[r]},`${i}<placeholder index="${t}"></placeholder>`}return i+s[r]}return i})).join(""),o=i.parseFromString(r,"text/html").querySelector("body");[...o.querySelectorAll("placeholder")].forEach((t=>{t.replaceWith(n[t.getAttribute("index")].frag)}));const d=new DocumentFragment;return d.appendChild(...o.childNodes),d}}class n extends s{constructor(t,e){super(),this.dataSet=t,this.widgetName=e}renderRemoveButton(t){const{x:e}=Pangolicons.icons;return t?"":`<button class="widget-remove" widget-id="${this.id}">\n\t\t${e.toString({"stroke-width":2})}\n\t</button>`}render({preview:t=!1}={}){const e=this.html,[i,...s]=this.dataSet.title.split(" "),{description:n,value:r,lastValue:o,threshold:d,unit:a}=this.dataSet;return e`
			<div
			...
			>
				...
				<div class="widget-content">
					<div class="widget-content__trend">
						${this.renderTrend(r,o)}
						${this.renderValue(r,d,a)}
					</div>
					<div class="widget-content__details">
						${this.renderDetails({value:r,lastValue:o,threshold:d,unit:a})}
					</div>
				</div>
			</div>
